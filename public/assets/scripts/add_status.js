...
// Thêm code mới cho Course
document
  .querySelector('button[name="add_course"]') // Giả sử bạn có nút này trong HTML
  .addEventListener("click", function () {
    // Create the pop-up modal
    let modal = document.createElement("div");
    modal.style.position = "fixed";
    ...
    form.innerHTML = `
      <form id="courseForm">
          <h2 class="text-xl font-bold mb-4">Thêm khóa học mới</h2>
          <div class="grid grid-cols-2 gap-4">
              <div>
                  <label for="course_id" class="block text-sm font-medium text-gray-700">ID:</label>
                  <input type="text" id="course_id" name="course_id" required 
                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              </div>
              <div>
                  <label for="course_name" class="block text-sm font-medium text-gray-700">Tên khóa học</label>
      ...
      fetch("/api/add_course", {
        ...
        .then((data) => {
          if (data.message === "Course added successfully") {
            ...
            const courseData = data.newCourse;
            const courseTableBody = document.getElementById("course-table-body");
            const row = document.createElement("tr");
            row.classList.add("text-gray-700", "dark:text-gray-400");
            row.innerHTML = `
                <td class="px-4 py-3 text-sm">${courseData.course_id}</td>
                <td class="px-4 py-3 text-sm">${courseData.course_name}</td>
                <td class="px-4 py-3 text-sm">
                    <button class="p-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" type="submit" onclick="editCourse(${courseData.course_id})">Edit</button>
                </td>
            `;
...
// Hàm editCourse (có thể thêm nếu cần)
function editCourse(courseId) {
  // Xử lý chỉnh sửa khóa học tại đây
