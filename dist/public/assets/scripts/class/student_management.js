function addGrade(studentId, classId) {
  ...
  form.innerHTML = `
        <h2 class="text-xl font-bold mb-5 text-center">${
          t?.class?.score?.add_score?.title || "Add Grade"
        }</h2>
        ...
        <div class="mb-4">
            <label for="grade" class="block text-sm font-medium text-gray-700 mb-1">${
              t?.class?.score?.add_score?.grade_label || "Grade"
            }</label>
            <input type="text" id="grade" name="grade" required
                class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-base">
        </div>
        <div class="mt-6 flex justify-end space-x-3">
            <button type="button" id="closeModal" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-md text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">${
              t?.class?.cancel_button || "Cancel"
            }</button>
            <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-md text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">${
              t?.class?.score?.add_score?.save_button || "Save"
            }</button>
        </div>`;  form.querySelector("#closeModal").addEventListener("click", function () {
    ...
    fetch("/api/transcript", {
      ...
      .then((data) => {
        ...
        if (data.message.includes("successfully") && data.createdtranscript) {
          Swal.fire({
            icon: "success",
            title: `${
              t?.class?.score?.add_score?.swal?.success_title || "Success"
            }`,
            text: `${
              t?.class?.score?.add_score?.swal?.success_text ||
          ...
          for (const r of rows) {
            ...
            if (
              idCell &&
              idCell.textContent &&
              idCell.textContent.trim() === studentIdStr
            ) {
              row = r;
              break;
            }
          ...
          if (row) {
            ...
            if (editGradeCell) {
              ...
              editGradeCell.innerHTML = `
                <button class="px-2 py-1 text-sm font-medium text-white bg-blue-600 rounded hover:bg-blue-700" onclick="editGrade(${
                  newData.student_id
                }, ${newData.transcript_id}, '${newData.grade}')">${
            ...
            Swal.fire({
              icon: "error",
              title: `${
                t?.class?.score?.add_score?.swal?.error_title || "Error"
              }`,
              text: `${
                t?.class?.score?.add_score?.swal?.error_text ||
                "Student not found in table"
              }`,
      ...
      .catch((error) => {
        ...
        Swal.fire({
          ...
          text: `${
            t?.class?.score?.add_score?.swal?.error_text || "Error adding grade"
          } ${error.message || ""}`,
          confirmButtonText: t?.class?.close_button || "Close",
        });
      });
...
async function deleteStudent(studentId) {
  ...
  const result = await Swal.fire({
    ...
    text:
      t?.class?.score?.delete_student?.swal?.text ||
    ...
    confirmButtonText:
      t?.class?.score?.delete_student?.swal?.confirm_button || "Delete!",
  ...
  try {
    ...
    if (response.ok) {
      ...
      Swal.fire({
        icon: "success",
        title:
          t?.class?.score?.delete_student?.swal?.success_title || "Success!",
        text:
          (t?.class?.score?.delete_student?.swal?.success_text ||
            "Student deleted: ") + data.message,
      ...
      Swal.fire({
        ...
        text:
          errorData.message ||
          t?.class?.score?.delete_student?.swal?.error_text ||
...
async function cancel(registrationId, studentId) {
  ...
  const result = await Swal.fire({
    ...
    text:
      t?.class?.score?.delete_confirm?.text ||
    ...
    cancelButtonColor: "#3085d6",
    confirmButtonText:
      t?.class?.score?.delete_confirm?.confirm_button || "Cancel Registration",
    cancelButtonText: t?.class?.cancel_button || "Go Back",
    input: "textarea", // Use a textarea for the reason input
    inputPlaceholder:
      t?.class?.score?.delete_confirm?.reson || "Enter reason for cancellation",
    inputAttributes: {
      "aria-label":
        t?.class?.score?.delete_confirm?.input_aria ||
    ...
    preConfirm: (reason) => {
      if (!reason || reason.trim() === "") {
        Swal.showValidationMessage(
          t?.class?.score?.delete_confirm?.reason_required ||
  ...
  try {
    ...
    const response = await fetch(
      `/api/class_registation/${registrationId}?lang=${currentLang}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      Swal.fire({
        ...
        text:
          errorData.message ||
          t?.class?.score?.delete_confirm?.error_text ||
...
async function editGrade(studentId, transcriptId, grade) {
  ...
  form.innerHTML = `
      <h2 class="text-xl font-bold mb-5 text-center">${
        t?.class?.score?.edit_score?.title || "Edit Grade"
      }</h2>
      <div class="mb-4">
        <label for="grade" class="block text-sm font-medium text-gray-700 mb-1">${
          t?.class?.score?.edit_score?.label || "Grade"
        }</label>
      ...
      <div class="mt-6 flex justify-end space-x-3">
        <button type="button" id="closeModal" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-md text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">${
          t?.class?.score?.edit_score?.close_button || "Close"
        }</button>
        <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-md text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">${
          t?.class?.score?.edit_score?.save_button || "Save"
        }</button>
      </div>
    `;
      ...
      .then((data) => {
        if (data.message.includes("successfully")) {
          Swal.fire({
            icon: "success",
            title:
              t?.class?.score?.edit_score?.swal?.success_title || "Success!",
            text:
              t?.class?.score?.edit_score?.swal?.success_text ||
          ...
          if (row) {
            ...
            if (editGradeCell) {
              editGradeCell.classList.remove("hidden");
              editGradeCell.innerHTML = `<button class="px-2 py-1 text-sm font-medium text-white bg-blue-600 rounded hover:bg-blue-700" onclick="editGrade(${studentId}, ${transcriptId}, '${
                data.updatedtranscript.grade
              }')">${t?.class?.edit_grade_button || "Edit Grade"}</button>`;
            }
          } else {
            Swal.fire({
              icon: "error",
              ...
              text:
                t?.class?.score?.edit_score?.swal?.error_text ||
